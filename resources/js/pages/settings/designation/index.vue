<template>
    <div class="container mx-auto max-w-3xl">
        ...
        <div class="flex justify-between items-center w-full mb-4">
            <h1 class="page-heading">{{ $t('designation') }}</h1>

            <a @click="open('createDesignation')" class="button-primary cursor-pointer">
                <icon name="plus-sm" />
        ...
        <div class="relative">
            ...
            <ul class="mt-2 border bg-white border-gray-100 dark:border-gray-800 dark:bg-gray-900 rounded-lg overflow-hidden" v-show="designations && designations.length">
                <li class="py-2 px-4 flex justify-between items-center bg-gray-300 font-semibold">
                    <span>Name</span>
                    <span>Action</span>
                </li>
                <li v-for="(data, key) in designations" :key="key" class="py-2 px-4 flex justify-between items-center border-b border-gray-10">
            ...
            <div v-show="!loading && designations && !designations.length" class="text-center py-8">
...
import DesignationCreate from '~/components/designation/create'
...
export default {
    ...
    metaInfo () {
        return { title: this.$t('designation') }
    },
    ...
    // Map Getters
    computed: {
        ...mapGetters('designation', ['designations','loading','pagination']),
    },
  ...
  methods: {
    ...
    // GET ALL ROLE  
    async getData() {
        this.$store.state.designation.loading=true;
        await this.$store.dispatch('designation/fetchDesignation', this.pagination.current_page);
    },
