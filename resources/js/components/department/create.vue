<template>
    <modal name="createDepartment">
        <div slot="header">
            <h2 class="text-xl text-gray-600 font-medium">Add new department</h2>
        </div>
        <div slot="modal-form">
...
export default {
    ...
    methods: {
        close() {
			this.$store.dispatch("modals/close", 'createDepartment')
        },
        
        // save role
        async save () {
            await this.form.post(window.location.origin+'/api/department')
            .then((response)=>{
